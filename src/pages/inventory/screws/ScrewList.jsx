...
const ScrewList = () => {
  ...
  const openDeleteModal = (item) => {
    setDeleteId(item.screw_id);
    setDeleteName(`${item.color} screw`);
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const result = await deleteScrew(deleteId);
      if (result?.success) {
        toast.success("Screw deleted successfully!");
        await fetchData();
      } else {
        toast.error(result?.message || "Failed to delete screw.");
      }
    } catch (err) {
      ...
      Cell: ({ row }) => (
        <div className="flex items-center justify-center gap-1">
          <Button icon="ph:pencil-simple" className="btn-warning h-9 w-9 p-0"
            onClick={() => navigate(`/inventory/screws/edit/${row.original.screw_id}`)} />
