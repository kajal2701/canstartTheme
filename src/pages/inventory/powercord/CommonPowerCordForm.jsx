...
import { POWER_CORD_TYPES } from "@/utils/constants";
...
const CommonPowerCordForm = ({ isEdit = false, initialData = {}, onSubmit, onCancel, title }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      else { toast.success(`Power cord ${isEdit ? "updated" : "added"}!`); navigate("/inventory/powercord"); }
    } catch { toast.error(`Failed to ${isEdit ? "update" : "add"} power cord`); }
    finally { setIsSubmitting(false); }
  };

  const handleCancel = () => { onCancel ? onCancel() : navigate("/inventory/powercord"); };

  return (
    <>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">{title}</h1>
        <Button text="Back to Power Cords" icon="ph:arrow-left" className="btn-outline-primary" onClick={handleCancel} />
      </div>
      <Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <Select value={formData.type} onChange={(e) => handleInputChange("type", e.target.value)}
                options={POWER_CORD_TYPES} placeholder="Select Type" error={errors.type} />
            </div>
            <div>
