...
import { getControllers, deleteController } from "@/services/inventoryService";
...
const ControllerList = () => {
  ...
  const fetchData = async () => {
    ...
    try { setData(await getControllers()); }
    catch { setError("Failed to load controllers. Please try again."); }
    finally { setLoading(false); }
  };
  useEffect(() => { fetchData(); }, []);

  const openDeleteModal = (item) => { setDeleteId(item.controller_id); setDeleteName(item.type); setDeleteModalOpen(true); };
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const result = await deleteController(deleteId);
      if (result?.success) { toast.success("Controller deleted!"); await fetchData(); }
      else toast.error(result?.message || "Failed to delete.");
    } catch { toast.error("Failed to delete controller."); }
      ...
      Cell: ({ row }) => (
        <div className="flex items-center justify-center gap-1">
          <Button icon="ph:pencil-simple" className="btn-warning h-9 w-9 p-0"
            onClick={() => navigate(`/inventory/controllers/edit/${row.original.controller_id}`)} />
  ...
  return (
    <>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        ...
        <Button text="Add Controller" icon="ph:plus" className="btn-primary w-full sm:w-auto"
      ...
      <Card className="overflow-hidden">
        {error ? (
          ...
          <div className="overflow-x-auto">
            <DataTable title="Controllers List" columns={columns} data={data} loading={loading} className="min-w-[600px]" />
          </div>
        )}
      </Card>
