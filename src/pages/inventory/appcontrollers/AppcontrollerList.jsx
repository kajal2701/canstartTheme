...
import { getAppcontrollers, deleteAppcontroller } from "@/services/inventoryService";

const AppcontrollerList = () => {
  ...
  const fetchData = async () => {
    ...
    try { setData(await getAppcontrollers()); }
    catch { setError("Failed to load app controllers. Please try again."); }
    finally { setLoading(false); }
  };
  useEffect(() => { fetchData(); }, []);

  const openDeleteModal = (item) => { setDeleteId(item.appcontroller_id); setDeleteName(item.type); setDeleteModalOpen(true); };
  ...
  const handleConfirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const result = await deleteAppcontroller(deleteId);
      if (result?.success) { toast.success("App Controller deleted!"); await fetchData(); }
      else toast.error(result?.message || "Failed to delete.");
    } catch { toast.error("Failed to delete app controller."); }
      ...
      Cell: ({ row }) => (
        <div className="flex items-center justify-center gap-1">
          <Button icon="ph:pencil-simple" className="btn-warning h-9 w-9 p-0"
            onClick={() => navigate(`/inventory/appcontrollers/edit/${row.original.appcontroller_id}`)} />
    ...
    <>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            <Icon icon="ph:device-mobile" className="text-xl text-lime-600" />
            <h1 className="text-xl font-bold">App Controllers</h1>
          </div>
        </div>
        {isAdmin && (
          <Button text="Add App Controller" icon="ph:plus" className="btn-primary w-full sm:w-auto"
            onClick={() => navigate("/inventory/appcontrollers/add")} />
        )}
      </div>
      <Card className="overflow-hidden">
        {error ? (
          ...
          <DataTable title="App Controllers List" columns={filteredColumns} data={data} loading={loading} />
        )}
      </Card>
...
export default AppcontrollerList;
