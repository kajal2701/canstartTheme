...
const CommonCableForm = ({ isEdit = false, initialData = {}, onSubmit, onCancel, title }) => {
  ...
  const types = [
    { value: "Coaxial Cable", label: "Coaxial Cable" },
    ...
    { value: "Audio Cable", label: "Audio Cable" },
  ];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      else { toast.success(`Cable ${isEdit ? "updated" : "added"}!`); navigate("/inventory/cables"); }
    } catch { toast.error(`Failed to ${isEdit ? "update" : "add"} cable`); }
    finally { setIsSubmitting(false); }
  };

  const handleCancel = () => { onCancel ? onCancel() : navigate("/inventory/cables"); };

  return (
    <>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">{title}</h1>
        <Button text="Back to Cables" icon="ph:arrow-left" className="btn-outline-primary" onClick={handleCancel} />
      </div>
      <Card>
